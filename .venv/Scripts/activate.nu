...
export-env {
    ...
    let virtual_env = r#'D:\Assigment1\medical-backend-flask\medical-backend\medical-backend\.venv'#
    let bin = r#'Scripts'#
    let path_name = if (has-env 'Path') { 'Path' } else { 'PATH' }
    let venv_path = ([$virtual_env $bin] | path join)
    let new_path = ($env | get $path_name | prepend $venv_path)
    let virtual_env_prompt = if (r#'medical-backend-py3.14'# | is-empty) {
        ($virtual_env | path basename)
    } else {
        r#'medical-backend-py3.14'#
    }
